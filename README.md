...
When this repository link is given directly to an AI, the first response must use the **full mobile-first Startup UI**. Do not use a compact fallback.

For a Russian-language conversation, the first response should follow this structure:
...
1. Do **not** preselect or visually privilege one language.
2. Do **not** show only language codes such as `RU · EN · FR · ES · UA`.
3. After Language selection, in the **same assistant response**, show the full **Step 2 of 3 — Style / Mode** screen.
4. Localize every Mode description to the selected interface language.
5. Show all eight startup choices with short descriptions and icons.
6. If Language changes before Style confirmation, re-render the complete Step 2 screen in the new language.
7. Never replace that transition with an acknowledgement-only reply.
8. A ты/вы or equivalent form-of-address change must not interrupt Startup Flow.
9. After Style selection, show **Step 3 of 3 — Confirmation** and wait for explicit confirmation.

Priority:

`Language selected → immediately render full Style Selection → wait for style choice`

The detailed canonical screens live in [system/START-HERE.md](system/START-HERE.md) and [system/STARTUP-FLOW-v1.0.md](system/STARTUP-FLOW-v1.0.md).
...
The first screen must show all five language names in a readable list, not only language codes. The second screen must show all eight startup styles with descriptions and icons.
